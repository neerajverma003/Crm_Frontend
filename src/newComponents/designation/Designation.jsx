...
const Designation = () => {
  ...
  const fetchDesignations = async () => {
    try {
      ...
      // Map populated company and department to string values
      const mappedData = data.map(desig => ({
        _id: desig._id,
        designation: desig.designation,
        companyName: typeof desig.company === "object" ? desig.company.companyName : desig.company,
        companyId: typeof desig.company === "object" ? desig.company._id : desig.company,
        departmentName: typeof desig.dep === "object" ? desig.dep.dep : desig.dep,
        departmentId: typeof desig.dep === "object" ? desig.dep._id : desig.dep,
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const departmentObj = departments.find(d => d._id === formData.departmentId);
      const newEntry = {
      ...
      setDesignations([newEntry, ...designations]);
  ...
  const filteredDesignations = designations.filter(d => {
    return (!filters.companyId || d.companyId === filters.companyId) &&
           (!filters.departmentId || d.departmentId === filters.departmentId);
  });

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-2xl font-semibold mb-6">Designation Form</h2>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              ...
              {departments.map(d => (
                <option key={d._id} value={d._id}>{d.dep}</option>
              ))}
            </select>
          </div>
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow-md border">
        <table className="min-w-full border-collapse">
          <thead className="bg-gray-100 border-b">
            <tr>
              <th className="p-3 text-left text-sm font-semibold text-gray-600">Designation</th>
              <th className="p-3 text-left text-sm font-semibold text-gray-600">Company</th>
          ...
          <tbody>
            {filteredDesignations.length === 0 ? (
              <tr>
                <td colSpan="4" className="p-3 text-center text-gray-500">No designations found</td>
              </tr>
            ) : (
              filteredDesignations.map((d) => (
                <tr key={d._id} className="border-b hover:bg-gray-50">
                  <td className="p-3 text-sm text-gray-800">{d.designation}</td>
                  <td className="p-3 text-sm text-gray-800">{d.companyName}</td>
                  <td className="p-3 text-sm text-gray-800">{d.departmentName}</td>
