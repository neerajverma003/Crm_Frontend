...
const AssignCompany = () => {
  ...
  const getAllAdmin = async () => {
    try {
      const response = await fetch("http://localhost:4000/getAdmin");
      const result = await response.json();
      setAdminList(result || []);
    } catch (error) {
      console.error(error);
  ...
  const handleAssign = async () => {
    ...
    try {
      const response = await fetch("http://localhost:4000/assign", {
        ...
        body: JSON.stringify({
          adminId: formData.selectedAdmin,
          companyIds: formData.assignedCompanies,
        }),
      });
      ...
      if (response.ok) {
        ...
        setFormData({ selectedAdmin: '', assignedCompanies: [] });
      } else {
